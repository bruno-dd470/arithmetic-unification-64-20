...
def main():
    ...
    print(f"Erreur moyenne test : {np.mean(test_err):.6f} %")
    print(f"Écart : {abs(np.mean(train_err) - np.mean(test_err)):.6f} %")
    print(f"Erreur max test : {np.max(test_err):.6f} %")
    print(f"Erreur min test : {np.min(test_err):.6f} %")
