...
# ============================================================
# Énergies des paires de bases ADN (eV)
# ============================================================
base_pairs = [
    ("A–T", 0.33),
    ("G–C", 0.49),
...
print("\nRecherche des meilleurs ajustements pour les paires de bases ADN")
print(f"Λₑ = {Lambda_e:.3f} eV, m ∈ [-5,5]\n")
print(f"{'Paire':<6} {'E_exp (eV)':>12} {'E_pred (eV)':>14} {'m':>3} {'Δ':>12} {'Err (%)':>10} {'ε actifs':>30}")
print("-" * 100)

for name, E_exp in base_pairs:
    m, delta, coeff, err = best_match(E_exp)
    if m is None:
        print(f"{name:<6} {E_exp:12.5f} {'--':>14} {'--':>3} {'--':>12} {'--':>10} {'--'}")
        continue
    E_pred = Lambda_e * (4**m) * delta
    err_pct = err * 100
    eps_str = format_coeffs(coeff)
    print(f"{name:<6} {E_exp:12.5f} {E_pred:14.6f} {m:3d} {delta:12.6f} {err_pct:9.5f}% {eps_str:30s}")