...
# ============================================================
# Énergies des interactions protéiques (eV)
# ============================================================
interactions = [
    ("Hélice α (H)", 0.25),
    ("Feuillet β (H)", 0.22),
...
print("\nRecherche des meilleurs ajustements pour les interactions protéiques")
print(f"Λₑ = {Lambda_e:.3f} eV, m ∈ [-5,5]\n")
print(f"{'Interaction':<16} {'E_exp (eV)':>12} {'E_pred (eV)':>14} {'m':>3} {'Δ':>12} {'Err (%)':>10} {'ε actifs':>30}")
print("-" * 100)

for name, E_exp in interactions:
    m, delta, coeff, err = best_match(E_exp)
    if m is None:
        print(f"{name:<16} {E_exp:12.5f} {'--':>14} {'--':>3} {'--':>12} {'--':>10} {'--'}")
        continue
    E_pred = Lambda_e * (4**m) * delta
    err_pct = err * 100
    eps_str = format_coeffs(coeff)
    print(f"{name:<16} {E_exp:12.5f} {E_pred:14.6f} {m:3d} {delta:12.6f} {err_pct:9.5f}% {eps_str:30s}")