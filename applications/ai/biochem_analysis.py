...
print("\nRecherche des meilleurs ajustements pour les énergies biochimiques")
print(f"Λₑ = {Lambda_e:.3f} eV, m ∈ [-5,5]\n")
print(f"{'Réaction / liaison':<20} {'E_exp (eV)':>12} {'E_pred (eV)':>14} {'m':>3} {'Δ':>12} {'Err (%)':>10} {'ε actifs':>30}")
print("-" * 110)

for name, E_exp in energies:
    m, delta, coeff, err = best_match(E_exp)
    if m is None:
        print(f"{name:<20} {E_exp:12.5f} {'--':>14} {'--':>3} {'--':>12} {'--':>10} {'--'}")
        continue
    E_pred = Lambda_e * (4**m) * delta
    err_pct = err * 100
    eps_str = format_coeffs(coeff)
    print(f"{name:<20} {E_exp:12.5f} {E_pred:14.6f} {m:3d} {delta:12.6f} {err_pct:9.5f}% {eps_str:30s}")